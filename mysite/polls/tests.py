...
class QuestionMethodTests(TestCase):
	def test_was_published_recently_with_future_question(self):
		"""shoul return FALSE for questions with future publish date"""
...
class QuestionViewTests(TestCase):
	def test_index_view_with_no_questions(self):
		...
		self.assertQuertsetEqual(response.context['latest_question_list'],[])

	def test_index_view_with_a_past_question(self):
		create_question(question_text="Past Question.", days=-30)
		response = self.client.get(reverse('polls:index'))
		self.assertQuertsetEqual(
			resonse.context['latest_question_list'],['<Question: Past Question.>']
			)
